...
import { ChevronLeftCircle, ChevronRightCircle, RefreshCcwIcon } from 'lucide-react'
...
const TabWrapper = ({ title, isSidebarOpen }: { title: string, isSidebarOpen: boolean }) => {
  return (
    <>
      {
      isSidebarOpen ? (
    ...
    />
        ...
        >
            <div className='flex w-full gap-4 lg:gap-6'>
              <Tab.List className={cn('flex flex-col divide-y divide-yellow-500 mt-4',
                isSidebarOpen ? 'w-44' : 'w-16',
                'transition-all duration-300 ease-out'
              )}>
                <div>
                  { isSidebarOpen ? <ChevronLeftCircle
                    strokeWidth={1}
                    className='h-8 w-8 cursor-pointer text-yellow-500 hover:text-yellow-300 mx-auto mb-4'
                    onClick={() => {
                  ...
                  /> : <ChevronRightCircle
                    ...
                    /> }
                  { isSidebarOpen ? <UserSelect onSelectUser={setUserId} /> : <div className='h-[72px]'></div> }
                </div>
